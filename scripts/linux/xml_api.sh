...
#wait # Wait for all management profiles to be created before attempting to make changes to interfaces
#for pid in "${splunk_service_pid_array[*]}"; do while [ -e "/proc/$pid" ]; do sleep 0.1; done; done && 

# Configure the interfaces with the correct management profiles
waits "${mgmt_profile_pid_array}" configure_interfaces &

#wait # Wait for all processes to finish before commiting
...
action "set" "2016 Docker/Remote Address Object" "$addr_object_xpath[@name='docker-remote']" "<ip-netmask>172.20.240.10/24</ip-netmask><description>Private IPv4 address for the '2016 Docker/Remote' server</description>" & address_object_pid_array+=($!)

# Create 2016 Docker/Remote NAT Address Object
action "set" "2016 Docker/Remote NAT Address Object" "$addr_object_xpath[@name='docker-remote-nat']" "<ip-netmask>172.25.$third_octet.97/24</ip-netmask><description>NAT IPv4 address for the '2016 Docker/Remote' server</description>" & address_object_pid_array+=($!)
...
while [ -e "/proc/$syslog_profile_pid" ]; do sleep 0.1; done && action "set" "Splunk Log Forwarding Profile" "$log_profiles_xpath[@name='Splunk']" "<match-list><entry name=\"syslog-traffic\"><send-syslog><member>syslog-server-profile</member></send-syslog><action-desc>Match all traffic logs and forward using syslog</action-desc><log-type>traffic</log-type><filter>All Logs</filter><send-to-panorama>no</send-to-panorama><quarantine>no</quarantine></entry><entry name=\"syslog-auth\"><send-syslog><member>syslog-server-profile</member></send-syslog><action-desc>Match all auth logs and forward using syslog</action-desc><log-type>auth</log-type><filter>All Logs</filter><send-to-panorama>no</send-to-panorama><quarantine>no</quarantine></entry><entry name=\"syslog-data\"><send-syslog><member>syslog-server-profile</member></send-syslog><action-desc>Match all data logs and forward using syslog</action-desc><log-type>data</log-type><filter>All Logs</filter><send-to-panorama>no</send-to-panorama><quarantine>no</quarantine></entry><entry name=\"syslog-decryption\"><send-syslog><member>syslog-server-profile</member></send-syslog><action-desc>Match all decryption logs and forward using syslog</action-desc><log-type>decryption</log-type><filter>All Logs</filter><send-to-panorama>no</send-to-panorama><quarantine>no</quarantine></entry><entry name=\"syslog-threat\"><send-syslog><member>syslog-server-profile</member></send-syslog><action-desc>Match all threat logs and forward using syslog</action-desc><log-type>threat</log-type><filter>All Logs</filter><send-to-panorama>no</send-to-panorama><quarantine>no</quarantine></entry><entry name=\"syslog-tunnel\"><send-syslog><member>syslog-server-profile</member></send-syslog><action-desc>Match all tunnel logs and forward using syslog</action-desc><log-type>tunnel</log-type><filter>All Logs</filter><send-to-panorama>no</send-to-panorama><quarantine>no</quarantine></entry><entry name=\"syslog-url\"><send-syslog><member>syslog-server-profile</member></send-syslog><action-desc>Match all url logs and forward using syslog</action-desc><log-type>url</log-type><filter>All Logs</filter><send-to-panorama>no</send-to-panorama><quarantine>no</quarantine></entry><entry name=\"syslog-wildfire\"><send-syslog><member>syslog-server-profile</member></send-syslog><action-desc>Match all wildfire logs and forward using syslog</action-desc><log-type>wildfire</log-type><filter>All Logs</filter><send-to-panorama>no</send-to-panorama><quarantine>no</quarantine></entry></match-list><description>Log profile for Splunk forwarding</description>" & log_forwarding_pid=$!
...
while [ -e "/proc/$log_forwarding_pid" ]; do sleep 0.1; done && action "edit" "Interzone-Default Security Policy" "$dsec_policy_xpath[@name='interzone-default']" "<entry uuid=\"22222222-2222-2222-2222-222222222222\" name=\"interzone-default\"><action>drop</action><log-start>no</log-start><log-end>yes</log-end><icmp-unreachable>yes</icmp-unreachable><log-setting>Splunk</log-setting></entry>" &
