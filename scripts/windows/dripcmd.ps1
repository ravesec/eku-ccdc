        ...
        "2" 
            {
                DO
                {
                    ...
                    switch($firewallchoice)
                    {
                        "1"
                        {
                            Write-Host -ForegroundColor Red -nonewline "`nWARNING: "
                            ...
                            class FirewallIP
                            {
                                ...
                                FirewallIP([string]$machineName, [string]$ipAddress)
                                {
                            ...
                            $firewallIps = @([FirewallIP]::new("windows2016","192.168.1.1")
                            )

                            foreach($ip in $firewallIps)
                            {
                                Write-Host $ip.machineName
                ...
                DO
                {
                    ...
                    switch($adchoice)
                    {
                        "1"
                        {
                            ...
                            Write-Host -ForegroundColor Red -nonewline "`nWARNING: "
                            Write-Host "This generates new passwords for all users in this Active Directory and outputs them into a .csv"
                            $warning = Read-Host "Are you sure you want to continue? (y/N)"
                            if ($warning -inotlike "y*")
                            {
                                Break
